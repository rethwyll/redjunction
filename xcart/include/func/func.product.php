 ...
 * @version    3039bf3eb094e2b4575469bee1921cf07373145a, v213 (xcart_4_6_1), 2013-07-26 16:43:32, func.product.php, random
...
class XCSearchProducts {
    ...
    public static function getSkipTablesByTemplate($tpl) { // {{{
        ...
        if (!isset($must_have_tables_with_includes)) {
            ...
            $must_have_tables_with_includes['modules/Recommended_Products/recommends.tpl'] = $must_have_tables_with_includes['customer/simple_products_list.tpl'];
            $must_have_tables_with_includes['modules/Bestsellers/bestsellers.tpl'] = $must_have_tables_with_includes['customer/simple_products_list.tpl'];
            $must_have_tables_with_includes['modules/Upselling_Products/related_products.tpl'] = $must_have_tables_with_includes['customer/simple_products_list.tpl'];
        }
    ...
    private static function getSelfMustHaveTablesByTemplate($tpl) { // {{{
        ...
        */
        ...
        $self_must_have_tables = array(
            ...
            'modules/Recently_Viewed/content.tpl' => array('images_T', 'pricing', 'products_lng_current'),
    ...
    if (!empty($active_modules['Product_Options'])) {
        ...
        if (empty($skip_tables['variants'])) {
            if ($current_area == 'C') {
                ...
                if (XCVariantsSQL::isOptimizationEnabled())
...
function func_select_product($id, $membershipid, $redirect_if_error = TRUE, $clear_price = FALSE, $always_select = XCProduct::SELECT_PRODUCT_CUSTOMER_MODE, $prefered_image_type = 'P')
{
    ...
    x_load_module('Product_Options');
    if ($current_area == 'C' && $always_select != XCProduct::SELECT_PRODUCT_ADMIN_PREVIEW) {
    ...
    $category = XCProductsCategoriesSQL::getProductCategory($id);
    if (!empty($category)) {
    ...
    if (
        !$single_mode
        && $current_area == 'C'
        && $always_select != XCProduct::SELECT_PRODUCT_ADMIN_PREVIEW
        && !empty($product)
    ) {
            ...
            if ($product_is_exists) {
                ...
                if (in_array($current_area, array('A','P'))) {
                    ...
                    if (
                        ...
                        && XCVariantsSQL::isOptimizationEnabled()
