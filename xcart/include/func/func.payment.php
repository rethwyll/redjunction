 ...
 * @version    77c5c46814ed430684d3e2145cd8915fc8b7c81c, v134 (xcart_4_6_1), 2013-08-26 17:55:46, func.payment.php, aim
...
function func_payment_get_capture_func($paymentid, $orderid = 0)
{
    ...
    if ($processor_file == 'ps_paypal_pro.php') {
        ...
        if (!empty($active_modules['XPayments_Connector']) && (empty($paypal) || $paypal['api'] != 'EC')) {
            func_xpay_func_load();
...
function func_payment_get_refund_func($paymentid, $orderid = 0)
{
    ...
    if ($processor_file == 'ps_paypal_pro.php') {
        ...
        } else if (!empty($active_modules['XPayments_Connector']) && (empty($paypal) || $paypal['api'] != 'EC')) {
            func_xpay_func_load();
...
function func_add_processor($processor, $orderby = 999)
{
    ...
    if (!zerolen($tmp['c_template'])) {
        ...
        $type_2_template = array (
            ...
            'X' => ('Y' == $config['XPayments_Connector']['xpc_use_iframe'])
...
function func_get_payment_countries() {
    global $sql_tbl;

    $return = array();

    $return['processors'] = func_query_hash("SELECT DISTINCT processor, country_code FROM $sql_tbl[payment_countries] WHERE country_code <> '' AND processor NOT LIKE 'ps_paypal%'",'processor', TRUE, TRUE);
    ...
    foreach ($return['processors'] as $processor => $proc_countries) {
        foreach ($return['processors'][$processor] as $country) {
            if (!array_key_exists($country, $return['countries'])) {
                $return['countries'][$country] = func_get_country($country);
            }
        }
    }
