 ...
 * @version    4e52975b17d5393244dab387b0c4bc845935cc54, v231 (xcart_4_6_1), 2013-09-11 15:58:07, func.cart.php, aim
...
function func_get_customer_zone_ship ($user, $provider, $type)
{
    ...
    if (is_array($zones)) {
        ...
        $tmp = func_query_column("SELECT zoneid FROM $sql_tbl[shipping_rates] WHERE zoneid IN ('" . implode("','", array_keys($zones)) . "') $provider_condition AND type='$type' GROUP BY zoneid ORDER BY NULL");
...
function func_calculate_taxes(&$products, $customer_info, $shipping_cost)
{
    ...
    if (!empty($active_modules['AvaTax'])) {
        $taxes = func_avatax_get_taxes($products, $customer_info, $shipping_cost);
            ...
            if (is_array($taxes['taxes'])) {
                ...
                foreach ($taxes['taxes'] as $k => $v) {
                    ...
                    if ($v['display_including_tax'] != 'Y') {
                        ...
                        $_display_discounted_subtotal_tax += $v['tax_cost_no_shipping'];
