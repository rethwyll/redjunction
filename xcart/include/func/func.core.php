 ...
 * @version    889e2fbec02742fa7f89d8a504259a0c100883e1, v551 (xcart_4_6_1), 2013-09-06 19:20:21, func.core.php, Alex
...
function func_display($tpl, &$templater, $to_display = true, $is_intermediate = false) { // {{{
    ...
    // Admin preview
    if (isset($_GET['is_admin_preview']) && $_GET['is_admin_preview'] == 'Y') {
...
function func_get_address_by_ip($ip) { // {{{
    ...
    if (!$is_200_ok) {
        // 1000 queries per hour limit in effect wait 5 min
        $geoip_request_delay = XC_TIME + SECONDS_PER_MIN*5;
        $response = false;
    } else {
 ...
 * @see    ____func_see____
 */
function func_refresh_product_rnd_keys($productid = null, $period = 86400)
{
