 ...
 * @version    f641e0d4ae27e74958884191b083813e9ecb8367, v215 (xcart_4_6_1), 2013-09-10 12:21:05, install.php, aim
...
function myquery($command)
{
    ...
    if (!isset($db_link) || !is_resource($db_link)) {
        ...
        mysql_query("SET NAMES '$sql_charset' COLLATE '$sql_collation'");
...
function is_allowed_query_upload($filename, $check_existing_modules = array()) {
    global $sql_tbl;
    static $installed_modules;

    if (empty($check_existing_modules))
        return true;
    ...
    if (!isset($installed_modules))
        ...
        implode("', '",array_keys($check_existing_modules))
        . "')");
    ...
    foreach($installed_modules as $module_name) {
        if (strpos($filename, $check_existing_modules[$module_name]) !== false) {
            return false;
        }
    }
...
function func_is_compatible_version()
{
    return 
        defined('COMPATIBLE_VERSION') 
            && (
                constant('COMPATIBLE_VERSION') == '4.6.1'
                || constant('COMPATIBLE_VERSION') == '4.6.x'
            );
}
...
*/
function get_secure_random_key($count) { //{{{
    global $xcart_dir;
