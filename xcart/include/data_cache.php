 ...
 * @version    8a7963ff3b715b03ffae7b3ae87d49e57b93a613, v124 (xcart_4_6_1), 2013-07-23 16:19:12, data_cache.php, aim
...
function func_force_data_cache_update() { // {{{
    global $config, $PHP_SELF, $sql_tbl;
    ...
    if (TRUE
        ...
        if (
            ...
            || (XC_TIME-$config['data_cache_expiration']) > DATA_CACHE_TTL
        ) {
            sleep(mt_rand(0,2));
            if ((XC_TIME-func_query_first_cell("SELECT value FROM $sql_tbl[config] WHERE name='data_cache_expiration'")) > DATA_CACHE_TTL) {
...
function func_sort_active_modules($a, $b) { // {{{
    static $sort_order = array(
        ...
        'Google_Checkout' => 10000, #can be unsetted in config/init.php;Google_Checkout depends on Egoods and XPayments_Connector it calls complexe function check_payment_methods from init.php
