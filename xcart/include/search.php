 ...
 * @version    28a9688b09ebf99698448474421e84a8e888b80b, v299 (xcart_4_6_1), 2013-08-27 16:48:33, search.php, aim
...
if ($mode == 'search') {
    ...
    if (
        ...
        && $current_area == 'C'
        && strpos(serialize($data), 'categories') === false
    ) {
    ...
    if (!$skip_categories_checking) {
        ...
        if (strpos(serialize($data), $sql_tbl['categories']) !== FALSE) {
    ...
    if (
            ...
            foreach ($products as $k => $v) {
                ...
                if (
                    ...
                    if (
                        ...
                        $products[$k]['variants_has_price'] = func_query_first_cell("SELECT COUNT($sql_tbl[pricing].variantid) FROM $sql_tbl[variants] INNER JOIN $sql_tbl[pricing] ON $sql_tbl[variants].productid = $sql_tbl[pricing].productid AND ".XCVariantsSQL::isVariantRow($v['productid'], 'pricing')."  AND $sql_tbl[pricing].price > 0") > 0;
                    }
        ...
        if (
            ...
            assert('is_null($smarty->get_template_vars("total_items")) && is_null($smarty->get_template_vars("first_item"))/*Previous navigation can be lost http://bugtracker.qtmsoft.com/view.php?id=42375*/');
