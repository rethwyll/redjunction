 ...
 * @version    5f1a604397f8c10c8992c116238bfe5503f0f67a, v154 (xcart_4_6_1), 2013-09-05 02:08:39, orders.php, Alex
...
if ($mode == 'search') {
    ...
    if (!empty($data['one_return_customer'])) {
        ...
        if (stripos($search_condition, 'GROUP BY') === FALSE) {
            $search_condition .= " GROUP BY $sql_tbl[orders].orderid";
        }
    ...
    if ($total_items > 0) {
        ...
        if ($orders) {
            ...
            if (empty($active_modules['XOrder_Statuses'])) {
                $smarty->register_modifier('order_status_color', 'trim');
            }
