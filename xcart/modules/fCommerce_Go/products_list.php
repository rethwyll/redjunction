 ...
 * @version    ca1bba9f4cb89f12d2bb56f202d7cea436a2aabe, v9 (xcart_4_6_1), 2013-06-07 12:16:56, products_list.php, aim
...
if (!$cat && $fb_mode != 'search') {
    ...
    if ($shop_configuration['bestsellers'] == 'Y' && $active_modules['Bestsellers'] && empty($expired)) {
        ...
        if ($categories_filter) {
            $additional_condition = " AND $sql_tbl[products_categories].categoryid IN ('" . implode("', '", $categories_filter) . "')";
        } else {
        ...
        if (empty($categories_filter)) {
            $_query['from_tbls'] = array('products' => ' USE INDEX(best_sellers) ');
            $_query['use_group_by'] = FALSE;
        }

        $_query['query'] = " AND $sql_tbl[products].sales_stats >= '0'" . $additional_condition;
        ...
        $bestsellers = func_search_products(
                $_query,
                @$user_account['membershipid'], // membership
                "$sql_tbl[products].sales_stats DESC, $sql_tbl[products].views_stats DESC", // orderby
        ...
        if (is_array($bestsellers)) {
            ...
            $search_data["products"]["sort_condition"] = "$sql_tbl[products].sales_stats DESC, $sql_tbl[products].views_stats DESC";
