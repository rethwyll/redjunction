 ...
 * @version    ce1105ea5a9f9f4a3f06ac9e6aef528e5dccf182, v54 (xcart_4_6_1), 2013-08-13 18:10:46, func.edit.php, aim
...
function func_recalculate_totals($cart)
{
    ...
    // Calculate taxes etc depending on the current store settings
        ...
        func_set_anonymous_userinfo($cart['userinfo'], 'skip_x_session_save');
    }
    ...
    if (isset($cart['use_discount_alt']) && $cart['use_discount_alt'] == 'Y') {
        ...
        if (is_array($products)) {
            foreach($products as $k => $product) {
                ...
                if (!empty($product['stock_update']) && $product['stock_update'] == 'Y') {
                    if (!func_aom_is_variant_changed($old_products[$k], $product)) {
    ...
    if (is_array($products)) {
        ...
        foreach ($products as $pk => $product) {
            ...
            if (!empty($product['itemid'])) {
                $query_data['itemid'] = $product['itemid'];
            }
