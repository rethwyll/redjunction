 ...
 * @version    360975f11638e59bcee8d1e06a7d3158242b89cc, v52 (xcart_4_6_0), 2013-05-17 11:37:20, import_variants.php, aim
 ...
 */
    ...
    foreach ($data_row as $key=>$variant) {
        $exists_variant = func_query_first("SELECT variantid, avail, weight, productcode, def FROM $sql_tbl[variants] WHERE productcode = '".addslashes($variant['variantcode'])."' AND productid = '$variant[productid]' AND " . XCVariantsSQL::isVariantRow());
...
// Export data
    ...
    while (($id = func_export_get_row($data)) !== false) {
        ...
        // Get data
        $mrow = db_query("SELECT $sql_tbl[variants].*, $sql_tbl[variants].variantid as image, $sql_tbl[pricing].price FROM $sql_tbl[products] ".XCVariantsSQL::getJoinQueryVariants()." INNER JOIN $sql_tbl[pricing] ON $sql_tbl[pricing].productid = $sql_tbl[variants].productid AND $sql_tbl[pricing].variantid = $sql_tbl[variants].variantid AND $sql_tbl[pricing].quantity = '1' AND $sql_tbl[pricing].membershipid = '0' WHERE $sql_tbl[variants].productid = '$id'".(empty($provider_sql) ? "" : " AND $sql_tbl[products].provider = '$provider_sql'"));
        if (
            !$mrow
            || db_num_rows($mrow) == 0
        ) {
